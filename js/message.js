...
function initLive2d (){
	...
	function moveDown(){
		...
		if (!isMobile && navigator.userAgent.indexOf("Firefox") > 0){
	...
	function move(){
		if(moveable){
			var ent = getEvent();
			var clientX = getEventX(ent);
			var clientY = getEventY(ent);
			var x = moveLeft + clientX - downX;
			var y = moveBottom +  (downY - clientY);
			var obj = smcc;
			obj.style.left = x + "px";
	...
	function moveUp(){
		if(moveable){
			$(document).unbind(moveEnentName,move);
			$(document).unbind(upEnentName,moveUp);
			if (isMobile){
				document.body.style.overflow='';//出现滚动条
			}
			var obj = smcc;
	...
	$(smcc).on(downEnentName, moveDown);
}
